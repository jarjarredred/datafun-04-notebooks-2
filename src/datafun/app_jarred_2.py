...
uv run python -m datafun.app_jarred
...
def correlation_matrix(df_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    # Using a nicer custom mask if you want, or just upgrading formatting:
    heatmap: Axes = sns.heatmap(
        correlation_matrix,
        annot=True,
        fmt=".2f",  # Explicitly format numbers to two decimal places
        cmap="coolwarm",
        center=0,
        square=True,  # Force cells to be perfectly square
        linewidths=0.5,  # Add subtle lines between squares
    )
    heatmap.set_title("Correlation Matrix Heatmap", pad=15)

    # Save Heatmap
    artifacts_dir = "artifacts"
    os.makedirs(artifacts_dir, exist_ok=True)
    heatmap_path = os.path.join(artifacts_dir, "figure_3_heatmap_jarred.png")
    plt.savefig(heatmap_path, dpi=300, bbox_inches="tight")
    LOG.info(f"Successfully saved heatmap to: {heatmap_path}")
...
def make_plots(df_clean: pd.DataFrame) -> None:
    """Create publication-quality plots and automatically save them to artifacts.

    Upgrades:
    - Sets a clean grid aesthetic
    - Adds transparency to scatter points to see density
    - Overlays a stripplot onto the box plot to show raw data point distribution
    """
    artifacts_dir = "artifacts"
    os.makedirs(artifacts_dir, exist_ok=True)

    # Set temporary clean styling for these plots
    sns.set_theme(style="whitegrid")

    # === Chart 1: Scatter Plot Enhanced ===
    LOG.info("---- Creating Enhanced Scatter Plot ------")
    plt.figure(figsize=(8, 6))

    # alpha=0.7 lets you spot where points overlap thickly
    scatter_plt: Axes = sns.scatterplot(
        data=df_clean,
        x="flipper_length_mm",
        y="bill_length_mm",
        hue=GROUP_COL,
        palette="Dark2",  # Use a distinct, punchy color palette
        alpha=0.7,
        s=60,  # Slightly larger point size
    )
    scatter_plt.set_xlabel("Flipper length (mm)", fontsize=11)
    scatter_plt.set_ylabel("Bill length (mm)", fontsize=11)
    scatter_plt.set_title("Flipper length vs Bill length (by species)", fontsize=13, pad=12)

    # Clean up legend placement
    plt.legend(title="Species", bbox_to_anchor=(1.05, 1), loc="upper left")

    # Save Figure 1
    fig1_path = os.path.join(artifacts_dir, "figure_1_jarred.png")
    plt.savefig(fig1_path, dpi=300, bbox_inches="tight")
    LOG.info(f"Successfully saved scatter plot to: {fig1_path}")


    # === Chart 2: Box Plot + Strip Plot Overlay ===
    LOG.info("------ Creating Box Plot with Data Point Overlay ---------")
    plt.figure(figsize=(8, 6))

    # 1. Draw the base boxplot with a bit of transparency
    box_plt: Axes = sns.boxplot(
        data=df_clean,
        x=GROUP_COL,
        y="flipper_length_mm",
        palette="Pastel2",  # Lighter background boxes
        boxprops=dict(alpha=0.6),  # Makes boxes softer so points stand out
        fliersize=0,  # Hide standard outliers since stripplot draws all points anyway
    )

    # 2. Overlay raw data points using stripplot
    sns.stripplot(
        data=df_clean,
        x=GROUP_COL,
        y="flipper_length_mm",
        hue=GROUP_COL,        # Color points by group
        palette="Dark2",      # Match scatter plot coloring
        size=4,
        jitter=0.2,           # Slightly jitter points horizontally so they don't stack on a line
        alpha=0.6,
        legend=False          # No duplicate legend needed
    )

    box_plt.set_xlabel("Species", fontsize=11)
    box_plt.set_ylabel("Flipper length (mm)", fontsize=11)
    box_plt.set_title("Distribution of Flipper Length by Species", fontsize=13, pad=12)

    # Save Figure 2
    fig2_path = os.path.join(artifacts_dir, "figure_2_jarred.png")
    plt.savefig(fig2_path, dpi=300, bbox_inches="tight")
    LOG.info(f"Successfully saved box plot to: {fig2_path}")

    # Reset theme defaults so it doesn't affect subsequent scripts in the pipeline
    sns.set_theme()
